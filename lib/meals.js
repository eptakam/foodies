...
// initialiser le package @aws-sdk/client-s3 pour pouvoir utiliser les fonctions de l'API S3 d'AWS
const s3 = new S3({ 
  region: 'ca-central-1' 
});
...
export async function saveMeal(meal) {
  ...
  // sauvegarder l'image dans un bucket S3 d'AWS
  const bufferedImage = await meal.image.arrayBuffer();
  ...
  */
  ...
  // sauvegarder l'image dans un bucket S3 d'AWS
  await s3.putObject({
      Bucket: 'emmataks-nextjs-demo-users-image',
      Key: fileName,
